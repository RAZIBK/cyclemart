...
router.post("/adminlogin", (req, res) => {
  adminhelpers
    ...
    .then((response) => {
      ...
      //  req.session.adm=response.admin
      // console.log(req.session.admin + "ad");
...
router.get("/adminhome",veryfyadminlogin,async function (req, res, next) {
  ...
  // console.log(":;;;;;;;;;;;;;;;;;;;;;;");
  // console.log(admin);
  // res.render("admin/adminindex", { layout:false, adminvalue, admin: true });
    [OrderCount, ProductCount] = await Promise.all([
   adminhelpers.getOrderCount(),adminhelpers.getProductCount()
    ])
...
router.post("/addBrandname", Storage.fields([{ name: "image1", maxCount: 1 },]),(req, res) => {
...
router.post("/addcategory",(req, res) => {
  ...
  adminhelpers.addcategory(req.body).then((response) => {res.redirect("/admin/addcategory");})
    .catch((error) => {
